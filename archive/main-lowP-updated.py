...
class AdaptiveLearningBenchmark:
    ...
    def run_evaluation(self, model_name: str, model_type: str, learner_profile: str, question_id: str, 
        ...
        if learner_profile == "profile_2" or learner_profile == "profile_5":
            ...
            messages_round2.append({"role": "assistant", "content": first_response})
            messages_round2.append({"role": "user", "content": "I don't understand"})
            
            # Get second response (no images needed for follow-up)
            result_round2 = inference.generate(messages=messages_round2, images=None)
            second_response = result_round2["response"]
            
            # Combine both responses
            combined_response = (
                f"[First Response]\n{first_response}\n\n"
                f"[User: I don't understand]\n\n"
                f"[Second Response]\n{second_response}"
            )
            ...
            evaluation_result = {
                ...
                "metadata": {
                    "round_1": {k: v for k, v in result.items() if k != "response"},
                    "round_2": {k: v for k, v in result_round2.items() if k != "response"}
                }
            }
        else:
            # Standard single-round processing for other profiles
            evaluation_result = {
                "timestamp": datetime.now().isoformat(),
                "model": model_name,
                "model_type": model_type,
    ...
    def run_evaluation_with_inference(self, inference, model_name: str, model_type: str, 
        ...
        if learner_profile == "profile_2" or learner_profile == "profile_5":
            ...
            # Prepare result with combined response
            evaluation_result = {
                "timestamp": datetime.now().isoformat(),
                "model": model_name,
                "model_type": model_type,
...
def main():
    ...
    models = [
        ...
        {"name": "Qwen/Qwen3-VL-30B-A3B-Instruct", "type": "llama"},
